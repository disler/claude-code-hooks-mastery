...
Name:`;
    ...
    try {
        ...
        let name = message.content[0]?.type === 'text' ? message.content[0].text.trim() : "Agent";
    ...
    if (args.length > 0) {
        if (args[0] === "--completion") {
            ...
            if (response) {
                ...
                console.log("Error calling Anthropic API");
            }
        }
    } else {
        console.log("Usage: node anth.ts 'your prompt here' or node anth.ts --completion or node anth.ts --agent-name");
    }
}
